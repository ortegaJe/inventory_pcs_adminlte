    ...
    <tr>
      ...
      <td>
          <form action="{{ route('machines.destroy', $machine->id) }}" method="POST">
            <a href="{{ route('machines.show', $machine->id) }}"><button type="button" class="btn bg-gradient-secondary btn-xs"><i class="fas fa-eye"></i></button></a>
            <a href="{{ route('machines.edit', $machine->id) }}"><button type="button" class="btn bg-gradient-success btn-xs"><i class="fas fa-edit"></i></button></a>
            @csrf
            @method('DELETE')
